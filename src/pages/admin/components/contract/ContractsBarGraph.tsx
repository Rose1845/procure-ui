...
const ContractsBarGraph: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const fetchDataForMonth = async (month: number) => {
                try {
                    const response = await axiosApi.get(`/contract/co/${month}`);
    ...
    return (
        <div className='flex flex-col items-center justify-end' key={chartKey}>
            <h2>Contract over time</h2>
            <div style={{ height: '400px', width: '600px' }}>
                <Line ref={ref1} data={chartData} options={{ maintainAspectRatio: false }} />
            </div>
        </div>
    );
};

export default ContractsBarGraph;
